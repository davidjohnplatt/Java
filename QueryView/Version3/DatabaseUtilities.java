...
public class DatabaseUtilities {
  ...
  private static String padString(String orig, int size,
    ...
    public static boolean jTableQueryResults(Connection connection,String query,boolean close) {
    try {
      DatabaseMetaData dbMetaData = connection.getMetaData();
      String productName = dbMetaData.getDatabaseProductName();
      String productVersion = dbMetaData.getDatabaseProductVersion(); 
      String qTitle = productName + " " + productVersion;    
      Statement statement = connection.createStatement();
      ResultSet resultSet = statement.executeQuery(query);  
      JTableDisplayResults d = new JTableDisplayResults(qTitle,resultSet);
 //   d.makeTable(resultSet);  -- now called from constructor to allow multiple signatures
      if (close) {
        connection.close();
      }
      return(true);
      ...
      String errString = sqle + "";
      ErrorFrame e = new ErrorFrame(errString);
      return(false);
    } 
  }
